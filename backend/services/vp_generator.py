...
def generate_value_propositions(db_session: Session, scan_session: ScanSession, claim_scores: list[ClaimScore], top_n: int = 3):
    ...
    for idx, claim in enumerate(selected_claims):
        ...
        vp = ValueProposition(
            ...
            tier_cds_score=claim.tier_cds_score,
            cds_zone=claim.cds_zone,
            crs_score=claim.crs_score,
            crs_believability=claim.crs_believability,
            crs_relevance=claim.crs_relevance,
            crs_fatigue_inverse=claim.crs_fatigue_inverse,
            crs_trigger_alignment=claim.crs_trigger_alignment,
