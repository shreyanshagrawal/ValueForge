  ...
  return (
    <section className="min-h-screen w-full relative overflow-x-hidden p-6" data-testid="page-theme-wrapper" style={{ background: "#0C0A18", fontFamily: "'Mulish', sans-serif" }}>
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto">
        <div className="relative" data-testid="page-scan-progress">
      <div className="relative max-w-[1280px] mx-auto px-6 lg:px-10 pt-14 pb-20">
        <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-10 items-start">
          ...
          <div className="animate-fade-up">
            ...
            <p className="mt-5 text-[15px] text-[#9CA3AF] leading-relaxed max-w-[58ch]">
            ...
            <div className="mt-6 flex flex-wrap gap-2 text-[12px]">
              ...
              <span className="px-3 py-1.5 rounded-full bg-[#211C2B]/80/40 text-[#9CA3AF] font-semibold border border-white/10">{PROJECT.brand} · {PROJECT.product}</span>
              <span className="px-3 py-1.5 rounded-full bg-[#211C2B]/80/40 text-[#9CA3AF] font-semibold border border-white/10">{PROJECT.targetTier} tier</span>
            </div>
            {/* Progress card */}
            <div className="mt-10 bg-[#211C2B]/80 backdrop-blur-[16px] rounded-2xl border border-white/10 shadow-[0_30px_60px_-30px_rgba(124,58,237,0.06)] p-7 lg:p-9">
              <div className="flex items-end justify-between">
                <div>
                  <div className="overline text-[#8A7CA8]">Overall progress</div>
                  <div className="mt-1 text-[44px] font-extrabold text-white leading-none tabular-nums" data-testid="overall-progress">
                    {overall}<span style={{ color: '#7C3AED' }}>%</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="overline text-[#8A7CA8]">Current stage</div>
                  <div className="mt-1 text-[15px] font-bold text-white uppercase tracking-[0.06em]" data-testid="current-stage-label">
                  ...
                  return (
                    <div key={s.key}
                      ...
                      className={`flex items-start gap-4 p-4 rounded-xl border transition-all
                        ${state === "active" ? "border-brand bg-[#211C2B]" : ""}
                        ${state === "done" ? "border-white/10 bg-[#211C2B]/50" : ""}
                        ${state === "pending" ? "border-white/10/40 bg-[#211C2B]/20 opacity-50" : ""}`}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline justify-between gap-3">
                          <span className={`text-[14px] font-bold uppercase tracking-[0.08em] ${state === "pending" ? "text-[#8A7CA8]" : "text-white"}`}>
                            {s.label}
                          </span>
                          <span className="text-[11px] uppercase tracking-[0.14em] text-[#8A7CA8]">
                            {state === "done" ? "Complete" : state === "active" ? "Running" : "Queued"}
                          </span>
                        </div>
                        <p className={`text-[13px] mt-0.5 ${state === "pending" ? "text-[#8A7CA8]" : "text-[#9CA3AF]"}`}>{STAGE_LABELS[s.key] || s.sub}</p>
                        {state === "active" && (
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              ...
              <span className="text-[12px] text-[#8A7CA8] ml-auto">
                Polling every <span className="font-semibold text-white">3s</span> · {fmt(SCAN_STAGES.length)} pipeline stages
              </span>
            </div>
          </div>
...
const Telemetry = ({ label, target, start }) => {
  ...
  return (
    <div className="rounded-xl bg-[#211C2B]/20 border border-white/10 p-3.5">
