      ...
      return (
        <button key={s.id}
          ...
          className={`w-full text-left px-3 py-2.5 rounded-md text-[13px] transition-all flex items-start gap-2
            ${active ? "text-white font-semibold" : "text-[#9CA3AF] hover:text-white hover:bg-[#211C2B]/20"}`}
...
const SectionCard = ({ s, index, onActivate }) => (
  <article
    ...
    className="group scroll-mt-[170px] rounded-2xl border border-white/10 bg-[#211C2B]/80 backdrop-blur-[16px] p-7 sm:p-9 card-hover shadow-sm"
  >
    <div className="flex items-start justify-between gap-4">
      <div className="flex items-center gap-3">
        ...
        <span className="overline text-[#8A7CA8]">Section</span>
      </div>
      <FileText size={16} className="text-[#8A7CA8] opacity-60 transition-colors" style={{}} onMouseEnter={e => e.currentTarget.style.color = '#7C3AED'} onMouseLeave={e => e.currentTarget.style.color = ''} />
    </div>
    <h2 className="mt-4 text-[26px] sm:text-[28px] font-extrabold leading-tight text-white">
      {s.title.replace(/^\d+\.\s*/, "")}
    </h2>
    <p className="mt-3 text-[15px] text-[#9CA3AF] leading-[1.75] max-w-[78ch] whitespace-pre-line">{s.body}</p>
  </article>
);

const ExportPanel = ({ onExport, exportState, countdown, formatTime }) => {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#211C2B]/80 backdrop-blur-[16px] p-6 shadow-sm" data-testid="export-panel">
      <span className="overline" style={{ color: '#7C3AED' }}>Export</span>
      <h3 className="text-[18px] font-bold text-white mt-1">Ship the full brief</h3>
      ...
      <div className="mt-5 space-y-2.5">
        <button
          ...
          className="w-full flex items-center justify-between rounded-lg text-white px-4 py-3.5 transition-all shadow-sm hover:shadow-md font-bold font-sans"
        ...
        <button
          ...
          className="w-full flex items-center justify-between rounded-lg border border-white/20 hover:border-white/40 bg-[#211C2B] px-4 py-3.5 transition-all shadow-sm hover:shadow-md"
        >
          <span className="flex items-center gap-2.5" style={{ color: '#7C3AED' }}>
            <span className="w-9 h-9 rounded-md inline-flex items-center justify-center" style={{ background: 'rgba(124,58,237,0.1)', color: '#7C3AED' }}><Download size={15} /></span>
            <span className="text-left">
              <span className="block text-[13.5px] font-bold uppercase tracking-[0.06em]">DOCX</span>
              <span className="block text-[11px] text-[#9CA3AF]">Editable Word document</span>
            </span>
          </span>
          <span className="text-[11px] text-[#9CA3AF]">~340 KB</span>
        </button>
      </div>
      ...
      <button
        ...
        className="mt-4 w-full inline-flex items-center justify-center gap-2 text-[12px] font-bold uppercase tracking-[0.1em] text-[#8A7CA8] hover:text-white py-2 border-t border-white/10 pt-3"
      >
...
const BrandBriefPreview = () => {
  ...
  return (
    <section className="min-h-screen w-full relative overflow-x-hidden p-6" data-testid="page-theme-wrapper" style={{ background: "#0C0A18", fontFamily: "'Mulish', sans-serif" }}>
      ...
      />
      <div className="absolute top-[5%] left-1/2 -translate-x-1/2 w-[800px] h-[600px] rounded-full bg-[#6B21A8]/20 blur-[150px] pointer-events-none z-0" />
      <div className="absolute bottom-[5%] right-0 w-[600px] h-[600px] rounded-full bg-[#EA580C]/10 blur-[150px] pointer-events-none z-0" />
      <div className="relative z-10 w-full max-w-[1400px] mx-auto">
        <div className="py-14" data-testid="page-brand-brief">
      {/* HERO */}
        ...
        <div className="relative grid lg:grid-cols-[1.4fr_0.8fr] gap-8 items-end">
          ...
          <div className="rounded-xl bg-white/10 border border-white/10 p-5 animate-fade-up" style={{ animationDelay: '120ms' }}>
            ...
            <button onClick={() => onExport("docx")} data-testid="hero-export-docx"
              className="mt-2 w-full inline-flex items-center justify-center gap-2 bg-white/10 border border-white/20 text-white font-bold uppercase text-[12.5px] tracking-[0.1em] py-3 rounded-md hover:bg-[#211C2B]/50 transition">
              Export DOCX
            </button>
          </div>
        </div>
      ...
      <div className="mt-10 grid lg:grid-cols-[220px_1fr_300px] gap-8 items-start">
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl p-6 text-center" style={{ border: '1px dashed rgba(124,58,237,0.2)', background: 'rgba(124,58,237,0.05)' }}>
            <span className="overline" style={{ color: '#7C3AED' }}>End of brief</span>
            <p className="text-[13.5px] text-[#9CA3AF] mt-2">
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border border-white/10 bg-[#211C2B]/80 backdrop-blur-[16px] p-6 shadow-sm">
            <span className="overline text-[#8A7CA8]">Brief stats</span>
            <ul className="mt-3 space-y-2.5 text-[13px] text-[#9CA3AF]">
              <li className="flex items-center justify-between"><span>Sections</span><b className="text-white">8</b></li>
              <li className="flex items-center justify-between"><span>Word count</span><b className="text-white">~1,420</b></li>
              <li className="flex items-center justify-between"><span>Charts referenced</span><b className="text-white">3</b></li>
              <li className="flex items-center justify-between"><span>Risk flags</span><b className="text-[#EF4444] font-bold">3</b></li>
